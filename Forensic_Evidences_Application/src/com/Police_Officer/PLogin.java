...
public class PLogin extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			ResultSet rs = st1
					.executeQuery("select * from tblregister_police where Email_Id='"
							+ emailid + "' and Password='" + password + "'");
			if (rs.next()) {

				session.setAttribute("pid", rs.getString("pid"));
				session.setAttribute("P_Email_Id", rs.getString("Email_Id"));
				session.setAttribute("P_Full_Name", rs.getString("Full_Name"));
				session.setAttribute("P_Mobile", rs.getString("Mobile"));
